...
同じ Wi-Fi につながっていれば、そのまま見られます。`python3 -m http.server` は最初から
外向きにも口を開けているので、追加の設定は要りません。

```sh
ipconfig getifaddr en0        # Mac の Wi-Fi アドレス。例 192.168.1.23
```

スマホのブラウザで `http://192.168.1.23:8899/` を開きます。

- 初回に macOS のファイアウォールが確認を出したら許可してください
- **Mac が寝ると止まります。** 見ている間は `caffeinate -i python3 -m http.server 8899` で起きたままにできます
- 縦画面では会社から始まります。指一本でなぞると移動、二本でつまむと拡大・縮小です

**外出先から見たい場合**は Tailscale を勧めます。自分の端末どうしだけを直接つなぐので、
作業内容が外に出ません。ngrok などの一時公開トンネルでも見られますが、**URL を知られると
誰でも中身を見られます。** 仕事の内容がそのまま出るので、使うなら承知の上で。
